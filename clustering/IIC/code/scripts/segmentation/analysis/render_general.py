...
parser.add_argument("--model_root", type=str, default="/usr/prakt/s0050/ss19_extendingpointnet/trained_models/IIC/")
...
for model_ind in model_inds:
  ...
  if "Coco" in config.dataset:
    ...
    if config.use_coarse_labels:
      all_label_names = [
        "unlabeled, pedestrians and cars",
        "road",
        "road lines and sidewalk",
  ...
  for imgs_dataloader_name in args.imgs_dataloaders:
    for net_name in net_names:
      ...
      if "Coco" in config.dataset:
        ...
        colour_map_gt = [np.array([0,0,0], dtype=np.uint8),
                         np.array([70,70,70], dtype=np.uint8),
                         np.array([190,153,153], dtype=np.uint8),
                         np.array([72,0,90], dtype=np.uint8),
                         np.array([220,20,60], dtype=np.uint8),
                         np.array([153,153,153], dtype=np.uint8),
                         np.array([157,234,50], dtype=np.uint8),
                         np.array([128,64,128], dtype=np.uint8),
                         np.array([244,35,232], dtype=np.uint8),
                         np.array([107,142,35], dtype=np.uint8),
                         np.array([0,0,142], dtype=np.uint8),
                         np.array([102,102,156], dtype=np.uint8),
                         np.array([220,220,0], dtype=np.uint8),
                        ]
      else:
      ...
      for b_i, batch in enumerate(imgs_dataloader):
        ...
        if "Coco" in config.dataset:
          ...
          render(orig_imgs, mode="image", name=("%d_img" % model_ind),
                 offset=next_img_ind,
                 out_dir=net_name_outdir)
