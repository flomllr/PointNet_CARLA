...
from code.datasets.segmentation import carla
...
# Added
def make_Carla_dataloaders(config):
  dataloaders = _create_dataloaders(config, carla.__dict__[config.dataset])

  mapping_assignment_dataloader = \
    _create_mapping_loader(config, carla.__dict__[config.dataset],
                           partitions=config.mapping_assignment_partitions)
...
def _create_dataloaders(config, dataset_class):
  ...
  for d_i in xrange(config.num_dataloaders):
    ...
    for train_partition in config.train_partitions:
      ...
      # Each train_img_curr is a dataset of the dataset_class (e.g. Potsdam or Carla)
      # Current dataset gets appended to train_img_list,
      # on which base a ConcatDataset will be created later.
      # In the case of Carla, there will be only one Partition and therefore only one element in train_imgs_list
