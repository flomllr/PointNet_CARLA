...
def labels_to_cityscapes_palette(image, flat=False):
    """
    Convert an image containing CARLA semantic segmentation labels to
    Cityscapes palette.
    """
    classes = cityscape_labels
    if not flat:
        array = image[:,:,0]#labels_to_array(image)
    else:
        array = image
...
def depth_to_local_point_cloud(image, color=None, max_depth=0.9, return_numpy=False):
    """
    Convert an image containing CARLA encoded depth-map to a 2D array containing
    ...
    Original implementation extended with return_numpy
    """
    ...
    pixel_length = 512 * 512
    u_coord = repmat(numpy.r_[512-1:-1:-1],
                     512, 1).reshape(pixel_length)
    v_coord = repmat(numpy.c_[512-1:-1:-1],
    ...
    # delete them
    max_depth_indices = numpy.where(normalized_depth > max_depth)
    normalized_depth = numpy.delete(normalized_depth, max_depth_indices)
    u_coord = numpy.delete(u_coord, max_depth_indices)
    v_coord = numpy.delete(v_coord, max_depth_indices)
    if color is not None:
        color = numpy.delete(color, max_depth_indices, axis=0)
    ...
    if return_numpy:
        return (numpy.transpose(p3d), color)
    ...
    return sensor.PointCloud(0, numpy.transpose(p3d))
