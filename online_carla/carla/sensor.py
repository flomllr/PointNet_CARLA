...
class Image(SensorData):
    ...
    def get_depth_live(self, percentile_99=1):
        """Save this image to disk (requires PIL installed)."""
        try:
            from PIL import Image as PImage
        except ImportError:
        ...
        # pdb.set_trace()
        grayscale = numpy.dot(image[:, :, :3], [1, 256.0, 256 * 256])
        grayscale /= (256.0 * 256.0 * 256.0 - 1.0)
        grayscale = numpy.clip(grayscale, 0, percentile_99) / percentile_99
        grayscale = grayscale / numpy.max(grayscale)
